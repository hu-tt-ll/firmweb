			...
			<div class="col-md-12 graphs">
				<div class="xs">
					...
					<div class="tab-pane active" id="horizontal-form">
						<form class="form-horizontal" id="myForm" method="post" action="${pageContext.request.contextPath }/reply/getAllReply">
							<input type="hidden" name="pageIndex" id="pageIndex" value="1"/>
							<div class="form-group">
								<label for="" class="col-sm-1 control-label">回复人</label>
								<div class="col-sm-2">
									<input type="text" class="form-control1" name="rname"  value="${rname}" placeholder="回复人">
								</div>
								<div class="col-sm-3">
							...
							<table class="table table-striped">
								...
								<c:forEach var="reply" items="${requestScope.replys}" varStatus="status">
									<tr>
										...
										<td>
											<span>${reply.rname}</span>
										</td>
										<td>
											<span>${reply.fnote}</span>
										</td>
										<td>
											<span>${reply.ftime}</span>
										</td>
										<td>
											<button type="button" onclick="todelete('${reply.id}')"  class="label btn_lg label-danger" style="padding:8px 12px">删除</button>
										</td>
									</tr>
								</c:forEach>
								</tbody>
...
<script>
    ...
    function dodelete(){
        ...
        $.getJSON("${pageContext.request.contextPath }/reply/delete",{id:id},function(data){
            if(data>0){
                location.href="${pageContext.request.contextPath }/reply/getAllReply";
			}
        });
    }
</script>
