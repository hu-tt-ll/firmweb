			...
			<div class="col-md-12 graphs">
				<div class="xs">
					...
					<div class="tab-pane active" id="horizontal-form">
						<form class="form-horizontal" id="myForm" method="post" action="${pageContext.request.contextPath }/post/getAllPost">
							...
							<table class="table table-striped">
								...
								<c:forEach var="post" items="${requestScope.posts}" varStatus="status">
									<tr>
										...
										<td>
											<span>
											<c:if test="${post.audit==0}">未审核</c:if>
											<c:if test="${post.audit==1}">已审核</c:if>
											</span>
										</td>
										<td>
											<c:if test="${post.audit==0}">
											<button type="button" onclick="toupdate('${post.id}')" class="label btn_lg label-primary" style="padding:8px 12px">审核</button>
											</c:if>
											<button type="button" onclick="todelete('${post.id}')"  class="label btn_lg label-danger" style="padding:8px 12px">删除</button>
										</td>
									</tr>
								</c:forEach>
								</tbody>
...
<script>
    ...
    function doupdate() {
        location.href="<%=Const.ROOT%>post/update/"+ $("#updateModal #id").val();
    }
    ...
    function dodelete(){
        ...
        $.getJSON("${pageContext.request.contextPath }/post/delete",{id:id},function(data){
            if(data>0){
                location.href="${pageContext.request.contextPath }/post/getAllPost";
			}
        });
    }
</script>
