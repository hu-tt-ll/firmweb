			...
			<div class="col-md-12 graphs">
				<div class="xs">
					...
					<div class="tab-pane active" id="horizontal-form">
						<form class="form-horizontal" id="myForm" method="post" action="${pageContext.request.contextPath }/user/getAllUser">
							...
							<table class="table table-striped">
								...
								<c:forEach var="user" items="${requestScope.users}" varStatus="status">
									<tr>
										...
										<td>
											<span>
											<c:if test="${user.audit==0}">未审核</c:if>
											<c:if test="${user.audit==1}">已审核</c:if>
											</span>
										</td>
										<td>
											<c:if test="${user.audit==0}">
											<button type="button" onclick="toupdate('${user.id}')" class="label btn_lg label-primary" style="padding:8px 12px">审核</button>
											</c:if>
											<button type="button" onclick="todelete('${user.id}')"  class="label btn_lg label-danger" style="padding:8px 12px">删除</button>
										</td>
									</tr>
								</c:forEach>
								</tbody>
...
<script>
    ...
    function doupdate() {
            location.href="<%=Const.ROOT%>user/update/"+ $("#updateModal #id").val();
    }
    ...
    function dodelete(){
        ...
        $.getJSON("${pageContext.request.contextPath }/user/delete",{id:id},function(){
                location.href="${pageContext.request.contextPath }/user/getAllUser";
